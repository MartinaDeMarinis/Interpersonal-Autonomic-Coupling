...
# =====================================================================
# LINEAR MIXED-EFFECTS MODELS - HRV
# =====================================================================
...
# Import and filter data (HF band: 0.15-0.40 Hz)
my_data <- read.csv(file_path, header = TRUE)
my_data <- subset(my_data, FREQUENCY >= 0.15 & FREQUENCY <= 0.40)
...
file_path <- file.path(script_dir, "results_HRV_condition.csv")
...
ggplot(my_data, aes(x = CONDITION, y = IPCC)) +
  ...
  labs(
    title = "IPCC in HRV across Conditions",
    x = "CONDITION",
    y = "IPCC"
  ) +
...
# =====================================================================
# GRAPH - GENDERS PAIRWISE COMPARISONS
# =====================================================================

message("=== GENDERS PAIRWISE COMPARISONS ===\n")
# Uploading GENDERS pairwise comparisons results
file_path <- file.path(script_dir, "results_HRV_genders.csv")

results_GENDERS_PAIRWISE <- read.csv(file_path, header = TRUE)
print(results_GENDERS_PAIRWISE)


ggplot(my_data, aes(x = GENDERS, y = IPCC)) +
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0, size = 1.2) +
  stat_summary(fun = mean, geom = "point", size = 3) +
  labs(
    title = "IPCC in HRV across GENDER COMBINATIONS",
    x = "GENDERS",
    y = "IPCC"
  ) +
  scale_x_discrete(labels = function(x) gsub(" ", "\n", x)) +
...
#conditions involving emotional stimui exposure: Paired Watching, Following, Leading.
...
interact_plot(
  ...
  labs(title = "Variation of IPCC in HRV with AROUSAL") +
